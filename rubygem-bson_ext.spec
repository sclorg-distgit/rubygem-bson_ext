...
Requires: %{?scl_prefix}rubygem(bson) >= 1.9.0
...
BuildRequires: %{?scl_prefix}rubygem(bson) >= 1.9.0
...
mkdir -p %{buildroot}%{gem_extdir_mri}/ext/%{gem_name}/%{gem_name}
cp -a .%{gem_dir}/* \
        %{buildroot}%{gem_dir}/

mv %{buildroot}%{gem_instdir}/ext/%{gem_name}/%{gem_name}/cbson.so \
  %{buildroot}%{gem_extdir_mri}/ext/%{gem_name}/%{gem_name}
